 ...
 */
public class L1S31N extends AppCompatActivity {
    ...
    public void onCreate(Bundle savedInstanceState){
        this.setTitle("Christopher Street-Sheridan Square");       //<---------------CHANGE THIS!!!!!!!!!!
    ...
    private void South(){
        startActivity(new Intent("com.example.android.nycsubway.L1S31S"));                    //<---------------CHANGE THIS!!!!!!!!!!
    }
    ...
    private class AsyncTaskData extends AsyncTask<Void, Void, ArrayList<String>> {
        private  ProgressDialog progDailog = new ProgressDialog(L1S31N.this);             //<---------------CHANGE THIS!!!!!!!!!!
            ...
            if (mWifi.isConnected()) {
                ...
                try {
                    ...
                    for (GtfsRealtime.FeedEntity entity : feed.getEntityList()) {
                        if (entity.getTripUpdate().getTrip().getRouteId().equals("1")) {
                            for (GtfsRealtime.TripUpdate.StopTimeUpdate stu : entity.getTripUpdate().getStopTimeUpdateList()) {
                                if (stu.getStopId().equals("133N")) {                              //<---------------CHANGE THIS!!!!!!!!!!
                ...
                if(datalist.size()==0){
                    datalist.add("No uptown times are available");
                }
                deleteFile("L1S31N");	//Uptown
                return datalist;
            }else{
                return ReadArrayListFromSD(L1S31N.this,"L1S31N");
            }
        }

        protected void onPostExecute(ArrayList<String> list1) {
            SaveArrayListToSD(L1S31N.this,"L1S31N",list1);
            ListView lv;
            progDailog.dismiss();
            lv = (ListView) findViewById(R.id.l1s1list);
            TextView tv;
            tv = new TextView(L1S31N.this);                       //<---------------CHANGE THIS!!!!!!!!!!
            tv.setText("Arrival Time(Uptown)");
            ...
            lv.addHeaderView(tv);
            ArrayAdapter<String> arrayAdapter = new ArrayAdapter<String>(L1S31N.this,R.layout.listviewlayout,list1 );             //<---------------CHANGE THIS!!!!!!!!!!
